...
def generate_pdf(name, zodiac, data, traits, dob, tob):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_auto_page_break(auto=True, margin=15)

    pdf.set_font("Arial", 'B', 16)
    pdf.cell(0, 10, f"{name}'s Horoscope", ln=True, align="C")

    pdf.set_font("Arial", '', 12)
    pdf.ln(10)
    pdf.cell(0, 10, f"Zodiac Sign: {zodiac.title()}", ln=True)

    pdf.ln(5)
    pdf.multi_cell(0, 10, f"🌟 Daily Horoscope:\n{data['general']}")
    pdf.ln(2)
    pdf.multi_cell(0, 10, f"❤️ Love: {data['love']}")
    pdf.multi_cell(0, 10, f"💼 Career: {data['career']}")
    pdf.multi_cell(0, 10, f"🩺 Health: {data['health']}")
    pdf.multi_cell(0, 10, f"🎨 Lucky Color: {data['color']}")
    pdf.multi_cell(0, 10, f"🔢 Lucky Number: {data['number']}")

    pdf.ln(5)
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(0, 10, "🧬 Personality Traits", ln=True)
    pdf.set_font("Arial", '', 12)
    pdf.multi_cell(0, 10, traits)

    pdf.ln(5)
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(0, 10, "🗺️ Birth Chart Summary", ln=True)
    pdf.set_font("Arial", '', 12)
    pdf.cell(0, 10, f"Date of Birth: {dob.strftime('%B %d, %Y')}", ln=True)
    pdf.cell(0, 10, f"Time of Birth: {tob.strftime('%I:%M %p')}", ln=True)
...
pdf_file = generate_pdf(name, zodiac, data, traits, dob, tob)
